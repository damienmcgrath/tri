...
export async function POST(request: Request) {
  ...
  const ipLimit = checkRateLimit("debrief-refresh-ip", ip, { maxRequests: 10, windowMs: 60_000 });
  if (!ipLimit.allowed) {
  ...
  const userLimit = checkRateLimit("debrief-refresh-user", user.id, { maxRequests: 5, windowMs: 60_000 });
