...
export async function POST(request: Request) {
  ...
  const ipLimit = await checkRateLimit("ftp-ip", ip, { maxRequests: 20, windowMs: 60_000 });
  if (!ipLimit.allowed) {
  ...
  const userLimit = await checkRateLimit("ftp-user", user.id, { maxRequests: 10, windowMs: 60_000 });
