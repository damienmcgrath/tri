...
function getDashboardDayToneClass(tone: "rest" | "upcoming" | "today-remaining" | "today-complete" | "completed" | "missed") {
  if (tone === "today-remaining") return "dashboard-week-segment--today-remaining";
  if (tone === "today-complete") return "dashboard-week-segment--today-complete";
  if (tone === "completed") return "dashboard-week-segment--completed";
  if (tone === "missed") return "dashboard-week-segment--missed";
  if (tone === "upcoming") return "dashboard-week-segment--upcoming";
  return "dashboard-week-segment--rest";
}

function getDashboardSessionCardClass(kind: "pending" | "completed" | "extra") {
  if (kind === "completed") return "dashboard-session-card dashboard-session-card--completed";
  if (kind === "extra") return "dashboard-session-card dashboard-session-card--extra";
  return "dashboard-session-card";
}

function getDashboardContextCardClass(ctaStyle: ContextualItem["ctaStyle"]) {
  return ctaStyle === "primary"
    ? "surface dashboard-context-card dashboard-context-card--primary p-5 md:p-6"
    : "surface dashboard-context-card dashboard-context-card--secondary p-5 md:p-6";
}
...
export default async function DashboardPage({
  ...
  if (!hasActivePlan && !hasAnyPlan) {
    return (
      <section className="dashboard-titanium titanium-theme space-y-4">
        <article className="surface dashboard-cluster p-6">
  ...
  return (
    <section className="dashboard-titanium titanium-theme space-y-4">
      <div className="grid gap-4 lg:grid-cols-[1.6fr_1fr]">
        <article className="surface dashboard-cluster p-5 md:p-6">
          ...
          <div className="mt-4 flex flex-wrap items-end justify-between gap-4">
            <div>
              <p className="dashboard-cluster-value text-5xl font-semibold leading-none">{completionPct}%</p>
            ...
            <span className={`signal-chip dashboard-status-pill ${resolvedStatusChip.className}`}>{resolvedStatusChip.label}</span>
          </div>
          ...
          <div className="dashboard-week-strip mt-5 grid grid-cols-7 gap-2">
            {dailyStates.map((day) => {
              return (
                <div key={day.iso} className={`dashboard-week-segment px-2 py-1.5 ${getDashboardDayToneClass(day.tone)}`}>
          ...
          <div className="dashboard-cluster-stats mt-4 grid grid-cols-3 gap-2 border-t pt-3 text-sm">
            <p className="dashboard-cluster-stat">Completed <span className="mt-0.5 block text-sm font-semibold">{toHoursAndMinutes(totals.completed)}</span></p>
            <p className="dashboard-cluster-stat">Remaining <span className="mt-0.5 block text-sm font-semibold">{toHoursAndMinutes(remainingMinutes)}</span></p>
            <p className="dashboard-cluster-stat">Missed <span className="mt-0.5 block text-sm font-semibold">{toHoursAndMinutes(missedMinutes)}</span></p>
          </div>
        </article>

        <article className="surface dashboard-support-card dashboard-today-card p-5 md:p-6">
                ...
                <div>
                  ...
                  <div className="space-y-2">
                    {pendingTodaySessions.map((session) => (
                      <div key={session.id} className={`${getDashboardSessionCardClass("pending")} px-3 py-2`}>
                  ...
                  <div>
                    ...
                    <div className="space-y-2">
                      {completedTodaySessions.map((session) => (
                        <div key={session.id} className={`${getDashboardSessionCardClass("completed")} px-3 py-2`}>
                          <p className="text-sm font-medium">{getSessionDisplayName(session)}</p>
                          <p className="text-xs text-muted">{getCompletedMinutes(session)} min • Done</p>
                        </div>
                      ))}
                      {extraTodayActivities.map((activity) => (
                        <Link key={activity.id} href={`/sessions/activity/${activity.id}`} className={`${getDashboardSessionCardClass("extra")} block px-3 py-2 transition hover:border-[hsl(var(--signal-load)/0.42)]`}>
      ...
      {contextualItems.length === 1 ? (
        <article className={getDashboardContextCardClass(contextualItems[0].ctaStyle)}>
          <p className="dashboard-kicker">{contextualItems[0].kicker}</p>
      ...
      {contextualItems.length === 2 ? (
        <div className="grid gap-4 lg:grid-cols-[1.6fr_1fr]">
          {contextualItems.map((item) => (
            <article key={item.kicker} className={getDashboardContextCardClass(item.ctaStyle)}>
              <p className="dashboard-kicker">{item.kicker}</p>
