...
export function ProgressGlanceCard({
  ...
  return (
    <Link href="#week-progress-details" className="block">
      <article className={`surface transition hover:border-[hsl(var(--fg)/0.22)] ${compact ? "p-3" : "p-4"}`}>
        <div className={`flex items-start ${compact ? "gap-2.5" : "gap-4"}`}>
          <div className={`relative flex shrink-0 items-center justify-center rounded-full border border-[hsl(var(--border))] bg-[hsl(var(--bg-card))] ${compact ? "h-11 w-11" : "h-14 w-14"}`}>
            <div
            ...
            />
            <div className={`relative flex items-center justify-center rounded-full bg-[hsl(var(--surface-1))] font-semibold ${compact ? "h-7 w-7 text-[10px]" : "h-10 w-10 text-xs"}`}>
              {ringPct}%
            </div>
          </div>
          ...
          <div className="min-w-0 flex-1">
            ...
            <div className="mt-1 flex flex-wrap gap-1.5 text-[11px]">
              <span className="signal-load inline-flex items-center rounded-full border px-2 py-0.5">Missed: {missedPlannedCount}</span>
              <span className="inline-flex items-center rounded-full border border-[hsl(var(--border))] bg-[hsl(var(--surface-2))] px-2 py-0.5 text-[hsl(var(--fg-muted))]">Extras: {unmatchedExtraCount}</span>
              <span className="inline-flex items-center rounded-full border border-[hsl(var(--border))] bg-[hsl(var(--surface-2))] px-2 py-0.5 text-[hsl(var(--fg-muted))]">Remaining: {remainingTimeLabel}</span>
            </div>
          </div>
