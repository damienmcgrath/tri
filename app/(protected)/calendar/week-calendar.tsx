...
function calendarDisciplineChipTone(sport: string) {
  const tones: Record<string, { bg: string; text: string; dot: string; border: string }> = {
    swim: { bg: "rgba(255,255,255,0.03)", text: "#F4F1EB", dot: "#BDC6D0", border: "rgba(255,255,255,0.08)" },
    bike: { bg: "rgba(255,255,255,0.03)", text: "#F4F1EB", dot: "#C2C9BD", border: "rgba(255,255,255,0.08)" },
    run: { bg: "rgba(255,255,255,0.03)", text: "#F4F1EB", dot: "#C9BEB4", border: "rgba(255,255,255,0.08)" },
    strength: { bg: "rgba(255,255,255,0.03)", text: "#F4F1EB", dot: "#C2BDD0", border: "rgba(255,255,255,0.08)" },
    other: { bg: "rgba(255,255,255,0.03)", text: "#F4F1EB", dot: "#C5CBD2", border: "rgba(255,255,255,0.08)" }
  };
...
function SessionActionMenu({
  ...
  return (
    <div className="relative" onClick={(event) => event.stopPropagation()}>
      <button
        type="button"
        className="race-lab-field rounded-md px-1.5 py-0.5 text-[11px] text-muted hover:text-foreground"
...
export function WeekCalendar({
  ...
  return (
    <section className="space-y-3">
      <header className="surface-subtle race-lab-calendar-toolbar flex flex-wrap items-center justify-between gap-2 px-3 py-2">
        ...
        <div className="flex flex-wrap items-center gap-2 text-xs">
          ...
          <select id="sport-filter" value={sportFilter} onChange={(e) => setSportFilter(e.target.value as SportFilter)} className="race-lab-field rounded-md px-2 py-1">
          ...
          <select id="status-filter" value={statusFilter} onChange={(e) => setStatusFilter(e.target.value as FilterStatus)} className="race-lab-field rounded-md px-2 py-1">
          ...
          <span className="status-badge-passive rounded-full px-2 py-0.5 text-[11px]">{completedCount} done · {plannedRemainingCount} remaining · {skippedCount} skipped · {extraSessionCount} extra</span>
        </div>
      </header>

      {hasAdaptation ? (
        <section className="race-lab-attention-panel px-3 py-2">
          ...
          <div className="flex flex-col gap-1.5 text-xs">
            {unmatchedUploads.map((upload) => (
              <div key={upload.id} className="race-lab-attention-row flex flex-col gap-1.5 px-2.5 py-2 md:flex-row md:items-center md:justify-between" data-issue="upload">
            ...
            {skippedToResolve.map((session) => (
              <div key={session.id} className="race-lab-attention-row flex flex-col gap-2 px-2.5 py-2 md:flex-row md:items-center md:justify-between" data-issue="skip">
                <div className="min-w-0">
            ...
            {movedItems.map((move) => {
              ...
              return (
                <div key={`move-${move.sessionId}`} className="race-lab-attention-row flex flex-col gap-2 px-2.5 py-2 md:flex-row md:items-center md:justify-between" data-issue="move">
                  <div className="min-w-0">
            ...
            {extraItems.map((item) => (
              <div key={`extra-${item.id}`} className="race-lab-attention-row flex flex-col gap-2 px-2.5 py-2 md:flex-row md:items-center md:justify-between" data-issue="extra">
                <div className="min-w-0">
      ...
      <article className="race-lab-calendar-board grid gap-2 lg:grid-cols-7">
        {weekDays.map((day) => {
          ...
          const dayState = isToday ? "today" : needsAttention ? "needs-attention" : metrics?.fullyDone ? "complete" : "planned";
          ...
          return (
            <section key={day.iso} className="race-lab-day-column surface-card h-full rounded-2xl border border-[hsl(var(--border))] p-2" data-day-state={dayState}>
              <div className="race-lab-day-header mb-2 min-h-[86px] border-b border-[hsl(var(--border))] pb-2">
                <p className="text-xs uppercase tracking-[0.14em] text-muted">{day.weekday}</p>
                <div className="flex items-center justify-between">
                  <p className="font-semibold">{day.label}</p>
                  {isToday ? <span className="rounded-full border border-[hsl(var(--accent-performance)/0.38)] bg-[hsl(var(--signal-recovery)/0.08)] px-2 py-0.5 text-[10px] text-[hsl(var(--signal-recovery))]">Today</span> : null}
                </div>
              ...
              <div className="space-y-1.5 pt-0.5">
                {daySessions.length === 0 ? (
                  <button onClick={() => setQuickAddDate(day.iso)} className="race-lab-empty-slot w-full min-h-[92px] rounded-xl border border-dashed border-[hsl(var(--border)/0.85)] px-2 py-2.5 text-xs text-muted hover:border-[hsl(var(--accent-performance)/0.38)] hover:text-accent">
                    + Add session
                ...
                {daySessions.map((session) => {
                  ...
                  const stateBadge =
                    state === "extra" ? (
                      <span className="inline-flex items-center gap-1 rounded-full border border-[hsl(var(--border))] bg-[hsl(var(--surface-soft))] px-1.5 py-0.5 text-[10px] text-muted">
                        <span aria-hidden="true" className="h-1.5 w-1.5 rounded-full bg-[hsl(var(--accent-performance))]" />
                        Extra
                      </span>
                  ...
                  return (
                    <article
                      key={session.id}
                      className={`race-lab-session-card rounded-xl px-2 py-1.5 text-xs transition ${reviewableCompleted ? "cursor-pointer hover:-translate-y-[1px] hover:border-[hsl(0_0%_100%/0.12)] hover:shadow-[0_12px_24px_-20px_hsl(0_0%_0%/0.9)] focus-visible:-translate-y-[1px] focus-visible:border-[hsl(0_0%_100%/0.12)] focus-visible:shadow-[0_12px_24px_-20px_hsl(0_0%_0%/0.9)] focus-visible:outline-none" : ""}`}
                      data-state={state}
                      onClick={() => {
                    ...
                    >
                      <div className="flex items-center justify-between gap-1">
                        <span className="race-lab-discipline-chip inline-flex items-center gap-1 rounded-full border px-1.5 py-0.5 text-[10px]" style={{ backgroundColor: disciplineTone.bg, color: disciplineTone.text, borderColor: disciplineTone.border }}>
...
function QuickAddModal({ initialDate, weekDays, onClose }: { initialDate: string; weekDays: WeekDay[]; onClose: () => void }) {
  ...
  return (
    <TaskModal onClose={onClose} title="Add session" description="Create a planned workout for this week.">
      ...
      >
        <select value={form.date} onChange={(e) => setForm((prev) => ({ ...prev, date: e.target.value }))} className="race-lab-field w-full rounded-lg px-3 py-2 text-sm">
          {weekDays.map((day) => <option key={day.iso} value={day.iso}>{day.weekday} · {day.label}</option>)}
        </select>
        <select value={form.sport} onChange={(e) => setForm((prev) => ({ ...prev, sport: e.target.value }))} className="race-lab-field w-full rounded-lg px-3 py-2 text-sm">
        ...
        <input value={form.type} onChange={(e) => setForm((prev) => ({ ...prev, type: e.target.value }))} placeholder="Workout title (optional)" className="race-lab-field w-full rounded-lg px-3 py-2 text-sm" />
        <input value={form.duration} onChange={(e) => setForm((prev) => ({ ...prev, duration: e.target.value }))} type="number" min={1} max={300} className="race-lab-field w-full rounded-lg px-3 py-2 text-sm" />
  ...
  return (
    <TaskSheet onClose={onClose} title={`Move ${getSessionTitle(session)}`} description="Move this planned session to a different day this week.">
      <div className="space-y-3">
        <select value={date} onChange={(e) => setDate(e.target.value)} className="race-lab-field w-full rounded-lg px-3 py-2 text-sm">
          {weekDays.map((day) => (
...
function AssignUploadModal({
  ...
  return (
    ...
    >
      <div className="space-y-3">
        <div className="race-lab-attention-row rounded-xl p-3" data-issue="upload">
        ...
        {candidateSessions.length === 0 ? (
          ...
          <select value={selectedSessionId} onChange={(e) => setSelectedSessionId(e.target.value)} className="race-lab-field w-full rounded-lg px-3 py-2 text-sm">
    ...
    >
      <div className="space-y-3 text-sm">
        ...
        {executionScore !== null && executionScoreBand ? (
          <div className="race-lab-session-row rounded-xl p-3">
        ...
        {session.notes ? <p className="race-lab-session-row rounded-lg p-2 text-xs text-muted">{session.notes}</p> : null}
...
function TaskSheet({ children, title, description, onClose }: { children: React.ReactNode; title: string; description?: string; onClose: () => void }) {
  return (
    <TaskOverlay onClose={onClose}>
      <aside className="race-lab-sheet relative ml-auto flex min-h-screen w-full max-w-xl flex-col border-l border-[hsl(var(--border))] shadow-2xl">
        <header className="race-lab-sheet-header border-b border-[hsl(var(--border))] px-5 py-4">
          <div className="flex items-start justify-between gap-3">
            ...
            <button type="button" onClick={onClose} className="race-lab-field rounded-md px-2 py-1 text-xs text-muted hover:text-foreground">Close</button>
          </div>
        </header>
  ...
  return (
    <TaskOverlay onClose={onClose}>
      <div className="relative z-10 flex min-h-screen items-center justify-center p-4">
        <section className="race-lab-modal w-full max-w-md rounded-2xl border border-[hsl(var(--border))] p-5 shadow-2xl">
          <header className="race-lab-modal-header mb-4 border-b border-[hsl(var(--border))] pb-3">
