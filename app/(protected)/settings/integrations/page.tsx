...
export default async function IntegrationsPage() {
  ...
  const [{ data: activities }, { data: links }, { data: plannedSessions }] = await Promise.all([
    uploadIds.length
      ? supabase
  ...
  return (
    <section className="space-y-4">
      ...
      <article className="surface p-5">
        <ActivityUploadsPanel initialUploads={uploads} plannedSessions={(plannedSessions as any[]) ?? []} />
      </article>
    </section>
  );
}
