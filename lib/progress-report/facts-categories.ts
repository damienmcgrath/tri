/**
 * Category-by-category fact builders for the progress report. Each function
 * is independent of the others and can be unit-tested separately. Kept in one
 * sibling because they all share the helper types and formatters from
 * `facts-helpers.ts`.
 *
 * Categories:
 * - Volume (per-sport minutes, completion %)
 * - Fitness (CTL trajectory)
 * - Pace-at-HR (per-discipline aerobic efficiency)
 * - Durability (decoupling on long endurance work)
 * - Peak performances (best run/bike/swim of the block)
 */
...
export function classifyFitnessDirection(
  delta: number,
...
export function buildFitnessTrajectory(
  blockStart: string,
  blockEnd: string,
...
export function inferPaceAtHrDirection(
  sport: "run" | "bike" | "swim",
...
export function summarizePaceAtHr(
  sport: "run" | "bike" | "swim",
  current: ProgressReportPaceAtHr["current"],
  prior: ProgressReportPaceAtHr["prior"],
...
export function buildDurabilityBlock(activities: ActivityRow[]) {
...
export function inferDurabilityDirection(
