/**
 * Intensity profile orchestration: fetches plan sessions, computes profiles
 * and weekly summaries, and persists them to the database.
 */
...
/**
 * Compute and persist intensity profiles + weekly summaries for all sessions
 * in a training plan. Returns counts of profiles and summaries written.
 */
export async function computeAndPersistIntensityProfiles(
  supabase: SupabaseClient,
  userId: string,
  planId: string
): Promise<{ profiles: number; summaries: number }> {
  ...
  return { profiles: profiles.length, summaries: summaries.length };
}
